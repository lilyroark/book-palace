...
<section class="container mt-3">
  <h2 class="pb-3 w-50 border-bottom border-3 fit-content" style="color: var(--medium-theme-color);border-color: var(--light-theme-color) !important;">My Reviews</h2>
  <div>
    <ul class="list-group list-group-flush w-25 p-3">
    <?php
    if (empty($reviews)) {
      echo "<p class='list-group-item'>No Reviews</p>";
    } else {
      foreach ($reviews as $review) {
        echo "<a class='list-group-item list-group-item-action'>{$review['ISBN']}</a>";
      }
    }
    ?>
    </ul>
    <a class="btn btn-dark mt-2" href="<?=$this->base_url?>/index.php?page=account&command=add_reviews">Add new review</a>
  </div>
</section>
